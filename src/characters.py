...
def clearTroops():
    ...
    troops_spawned['king'] = 0
    troops_spawned['queen'] = 0
...
# CLASSES FOR THE NEW TROOPS:
class SuperArcher:
    # mostly the copy of archers
    def __init__(self, position):
        self.speed = 1
        self.health = 100
        self.max_health = 100
...
class Healer:
    # mostly the copy of dragons
    def __init__(self, position):
        # updated the parameters based on the pdf
        self.speed = 1
        self.health = 250
        self.max_health = 250
        # heal property
    ...
    def move(self, troop):
        # movement based on the closest troop to it 
        if (self.alive == False):
            return
        index = troop.position
        troops = barbarians + balloons + dragons + archers + superArchers + kings + queens
        r = abs(index[0] - self.position[0])
        c = abs(index[1] - self.position[1])
        # i.e. if troop is within 7 radius no need to move and simply find the closest troop
        # used euclidian distance
        if((index[1]-self.position[1])**2+(index[0]-self.position[0])**2 <= 7**2):
            troop.health+=self.heal
            if(troop.health >= troop.max_health):
                troop.health = troop.max_health
        #    for tr in troops:
        #         if((index[1]-tr.position[1])**2 + (index[0]-tr.position[0])**2 <= 1**2):
        #            tr.health+=self.heal
        #         if(troop.health >= troop.max_health):
        #             troop.health = troop.max_health
            k=0
            while k < len(troops):
                ...
                if((index[1]-tr.position[1])**2 + (index[0]-tr.position[0])**2 <= 1**2):
                    tr.health += self.heal
                if(tr.health >= tr.max_health):
                    tr.health = tr.max_health
                k += 1
        # else move accordingly...
        else:
            if (r == 0):
                if (index[1] > self.position[1]):
                    for i in range(self.speed):
                        r = self.position[0]
                        c = self.position[1] + 1
                        self.position[1] += 1
                        if (abs(index[1] - self.position[1]) == 1):
                            break
                else:
                    for i in range(self.speed):
                        r = self.position[0]
                ...
                if (index[0] > self.position[0]):
                    for i in range(self.speed):
                        r = self.position[0] + 1
                        c = self.position[1]
                        self.position[0] += 1
                        if (self.position[0] == index[0]):
                            return
                else:
                    for i in range(self.speed):
                        r = self.position[0] - 1
...
def spawnSuperArcher(pos):
    ...
    # i.e. turn is_visible equal to true after 10 seconds so that defenses can now attack it.
    timer = thread.Timer(10, lambda: setattr(superArcher, 'is_visible', True))
    timer.start()
...
def move_superArchers(V, type):
    if (type == 1):
        for superArcher in superArchers:
            ...
            if (superArcher.target == None):
                superArcher.target = search_for_closest_building(
                    superArcher.position, V.map, 0)
            if (superArcher.target == None):
                continue
...
def move_healers(V):
    for he in healers:
        if (he.alive == False):
            continue
        closest_troop = search_for_closest_troop(he.position)
        if (closest_troop == None):
            continue
        he.move(closest_troop)


def search_for_closest_troop(pos):
    closestdist = 10000
    closesttroop = None
    troops = barbarians + balloons + superArchers + archers + dragons + kings + queens
    # basic algo to find and update the troop with minimum manhattan distance...
    for troop in troops:
        if (abs(pos[1]-troop.position[1])+abs(pos[0]-troop.position[0]) < closestdist and troop.health < troop.max_health):
            closesttroop = troop        
            closestdist = abs(pos[1]-troop.position[1])+abs(pos[0]-troop.position[0])
    return closesttroop
