...
def supArchColor(supArch):
    health = supArch.health
    max_health = supArch.max_health
    percentage = (health*100)/max_health
    if(percentage > 50):
        return Back.LIGHTMAGENTA_EX
    elif percentage > 20:
...
def printMap(V):
    ...
    for supArch in superArchers:
        a= 2*supArch.position[0]
        b = 2*supArch.position[1]
        # show super archers name only if is_visible equals true
        if supArch.is_visible == True:
            map_matrix[a][b] = supArchColor(supArch) + Fore.BLACK + 'SP'
            map_matrix[a+1][b] = supArchColor(supArch) + Fore.BLACK + '  '
            map_matrix[a][b+1] = supArchColor(supArch) + Fore.BLACK + 'AR'
            map_matrix[a+1][b+1] = supArchColor(supArch) + Fore.BLACK + 'CH'
    ...
    for dr in dragons:
        ...
        map_matrix[a+1][b] = drColor(dr) + Fore.BLACK + 'AG'
        map_matrix[a][b+1] = drColor(dr) + Fore.BLACK + 'ON'
    ...
    for bl in healers:
        ...
        map_matrix[a+1][b] = blColor(bl) + Fore.BLACK + '  '
        map_matrix[a][b+1] = blColor(bl) + Fore.BLACK + 'AL'
        map_matrix[a+1][b+1] = blColor(bl) + Fore.BLACK + 'ER'
